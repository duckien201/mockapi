...
export default function Screen2({ route, navigation }) {
    ...
    const loadCandidate = async () => {
        try {
            const response = await axios.get(`https://6610a85f0640280f219d41eb.mockapi.io/student/${id}`);
        ...
        Alert.alert('Xác nhận', 'Bạn có chắc chắn muốn xóa ứng viên này?', [
            { text: 'Không' },
            {
                text: 'Có', onPress: async () => {
                    try {
                        await axios.delete(`https://6610a85f0640280f219d41eb.mockapi.io/student/${id}`);
